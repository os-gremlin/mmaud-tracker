...
def load_xyz_and_intensity(path):
    ...
    # Optional: filter out all-zero rows (padding)
...
# ======================================================================
# MAIN PER-SEQUENCE PROCESSING
# ======================================================================

def process_sequence(seq_name):
    """Process a single sequence seqXXXX: generate all videos + PNGs."""
    seq_dir = os.path.join(ROOT_SEQ_DIR, seq_name)
    ...
    if not os.path.isdir(seq_dir):
        print(f"[{seq_name}] Missing sequence dir: {seq_dir}, skipping.")
        return
    if not os.path.isdir(lidar360_dir):
        print(f"[{seq_name}] Missing lidar_360 dir: {lidar360_dir}, skipping.")
        return
    if not os.path.isdir(avia_dir):
        print(f"[{seq_name}] Missing livox_avia dir: {avia_dir}, skipping.")
        return
    if not os.path.isdir(gt_dir):
        print(f"[{seq_name}] Missing ground_truth dir: {gt_dir}, skipping.")
        return
    ...
    # Make seq-specific result root and timestamp folder
    seq_result_root = os.path.join(RESULT_ROOT, seq_name)
    os.makedirs(seq_result_root, exist_ok=True)
    timestamp_run = datetime.now().strftime("%Y%m%d_%H%M%S")
    export_dir = os.path.join(seq_result_root, timestamp_run)
    ...
    if gt_ts.size == 0:
        print(f"[{seq_name}] No GT files found, skipping.")
        return
    ...
    if all_ts.size == 0:
        print(f"[{seq_name}] No timestamps found for animation, skipping.")
        return
    ...
    def compute_bounds():
        ...
        if not xyz_all:
            raise RuntimeError(f"[{seq_name}] No valid LiDAR points to compute bounds.")
    ...
    gt_timeline = np.vstack(gt_timeline)

    n_frames = len(all_ts)
    print(f"[{seq_name}] Using {n_frames} frames.")

    # ==================================================================
    # TRIPLE-VIEW ANIMATION
    # ==================================================================
    out_triple = os.path.join(export_dir, f"{seq_name}_triple_view_lidar360_avia_gt.{ext}")
    out_triple_ref = os.path.join(export_dir, f"{seq_name}_triple_view_lidar360_avia_gt_reflection.{ext}")
    ...
    with w_triple.saving(fig_triple, out_triple, DPI):
        for frame_idx in range(0, n_frames, EVERY):
            ...
            fig_triple.suptitle(
                f"{seq_name} Triple-View | Frame {frame_idx+1}/{n_frames}",
                fontsize=16
            )
            w_triple.grab_frame()
    ...
    with w_ref.saving(fig_tr, out_triple_ref, DPI):
        for frame_idx in range(0, n_frames, EVERY):
            t = all_ts[frame_idx]
            ...
            fig_tr.suptitle(
                f"{seq_name} Triple-View REFLECTION | Frame {frame_idx+1}/{n_frames}",
                fontsize=16
            )
            w_ref.grab_frame()
    plt.close(fig_tr)
    ...
    # ==================================================================
    # SINGLE-VIEW ANIMATIONS
    # ==================================================================
    out_l360_only = os.path.join(export_dir, f"{seq_name}_lidar360_only.{ext}")
    out_avia_only = os.path.join(export_dir, f"{seq_name}_avia_only.{ext}")
    out_gt_only   = os.path.join(export_dir, f"{seq_name}_gt_only.{ext}")
    ...
    with w_l360.saving(fig_l360, out_l360_only, DPI):
        for frame_idx in range(0, n_frames, EVERY):
            t = all_ts[frame_idx]
    ...
    with w_avia.saving(fig_avia_only, out_avia_only, DPI):
        for frame_idx in range(0, n_frames, EVERY):
            t = all_ts[frame_idx]
    ...
    out_png1 = os.path.join(export_dir, f"{seq_name}_lidar360_single_frame.png")
    fig_s1.savefig(out_png1, dpi=150)
    plt.close(fig_s1)
    ...
    out_png2 = os.path.join(export_dir, f"{seq_name}_avia_single_frame.png")
    fig_s2.savefig(out_png2, dpi=150)
    plt.close(fig_s2)
    ...
    out_png3 = os.path.join(export_dir, f"{seq_name}_gt_full_traj.png")
    fig_s3.savefig(out_png3, dpi=150)
    plt.close(fig_s3)

    print(f"[{seq_name}] Finished.")
    print("  Triple-view:          ", out_triple)
    print("  Triple-view reflected:", out_triple_ref)
    print("  LiDAR360 only:        ", out_l360_only)
    print("  Avia only:            ", out_avia_only)
...
# ======================================================================
# MAIN LOOP OVER ALL seqXXXX
# ======================================================================

def main():
    if not os.path.isdir(ROOT_SEQ_DIR):
        raise FileNotFoundError(f"ROOT_SEQ_DIR not found: {ROOT_SEQ_DIR}")

    seq_names = sorted(
        d for d in os.listdir(ROOT_SEQ_DIR)
        if d.startswith("seq") and os.path.isdir(os.path.join(ROOT_SEQ_DIR, d))
    )

    if not seq_names:
        print("No seqXXXX folders found under:", ROOT_SEQ_DIR)
        return

    print("Found sequences:", seq_names)
